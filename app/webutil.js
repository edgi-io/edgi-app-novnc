...
if (args._?.length) {
    const m = args._[0].match(/^vnc:\/\/(?:([\w%]*)(?::([\w%]+))?@)?([\.\w%]+|\[[:a-fA-F0-9]+\])(?::(\d+))?\/?(?:\?(.*))?$/);
    if (m) {
        let [_, user, password, host, port, extra] = m;
        if (host && !args['--host']) {
            args['--host'] = host;
        }
        if (port && !args['--port']) {
...
if (!args['--autoconnect'] && args['--host'] && args['--port']) {
    args['--autoconnect'] = 'true';
}

export function getArgvVar(name, defVal) {
    "use strict";
    const v = args['--' + name];
    //console.error('getArgvVar', name, v, defVal)
    return v ?? defVal ?? null;
}
