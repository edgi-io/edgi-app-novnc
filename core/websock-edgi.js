...
import { USyscall, UMarshal, AF, SOCK, MSG_NOSIGNAL, ErrorCode, readResolvConf } from '@edgi/sdk';
...
class EdgiSocket {
    constructor(uri, protocols, edgiTaskPromise) {
        const m = uri.match(/^wss?:\/\/([^:]+):(\d+)/);
        this.host = m[1];
    ...
    connect = async () => {
        ...
        this.syscall = window.edgi.syscall;
        const argv = window.edgi.task.argv;
        this.fd = await this.syscall.socket(AF.INET, SOCK.STREAM, 0);
        let addr = this.host;
        ...
        if (!addr.match(ipRegex)) {
            //const rc = await readResolvConf(window.edgi.fs);
            const [fd] = await this.syscall.socket(AF.UNIX, SOCK.DGRAM, 0);
            const a = UMarshal.marshalSockAddr({ family: AF.UNIX, path: '\0edgi-dns' });
            await this.syscall.connect(fd, a);
            const s = JSON.stringify({ req: addr });
            await this.syscall.sendto(fd, new TextEncoder().encode(s), 0, null);
            const [buf] = await this.syscall.recvfrom(fd, 2048, 0, 0, 0);
            const sbuf = new TextDecoder().decode(buf);
            const dbuf = JSON.parse(sbuf);
            if (dbuf.results?.length) {
                addr = dbuf.results[0];
            }
        }
        const port = this.port;
        const saddr = UMarshal.marshalSockAddr({ family: AF.INET, addr, port });
        await this.syscall.connect(this.fd, saddr);
    ...
    readloop = async () => {
        while (this.readyState === WebSocket.OPEN) {
            try {
                const [_len, data] = await this.syscall.pread(this.fd, this.rbuffer.length, -1);
                this.onmessage?.({ data });
            } catch(e) {
                console.error('socket readloop:', e);
                this.close();
            }
        }
    }
    ...
    send = async (data) => {
        ...
        while (written < data.length) {
            try {
                const [len] = await this.syscall.sendto(this.fd, data, null, MSG_NOSIGNAL);
                written += len;
            } catch(e) {
                if (e === -ErrorCode.EPIPE) {
                    break;
                } else {
                    throw e;
                }
    ...
    close = () => {
        ...
        this.syscall.close(this.fd).then(() => {
            this.fd = undefined;
...
export default class Websock {
    ...
    async flush() {
        if (this._sQlen > 0 && this.readyState === 'open') {
            const n = await this._websocket.send(this._encodeMessage());
    ...
    async sendString(str) {
        await this.send(str.split('').map(chr => chr.charCodeAt(0)));
    }
